    ...
    while (queue.length > 0) {
        ...
        for (let neighbor of this.getNeighbors(node, grid)) {  // Para cada vizinho do nó
            ...
            if (!visited.has(neighborStr)) {  // Se o vizinho não foi visitado
                ...
                let newPath = path.slice();  // Cria um novo caminho
                newPath.push(neighbor);  // Adiciona o vizinho ao novo caminho
                queue.push(newPath);  // Adiciona o novo caminho à fila
            }
        }
    }
...
getNeighbors: function(node, grid) {
    let neighbors = [];
    let directions = [[-1, 0], [1, 0], [0, -1], [0, 1]]; // Cima, baixo, esquerda, direita
    for (let direction of directions) {
        let neighbor = [node[0] + direction[0], node[1] + direction[1]];
        if (this.isValid(neighbor, grid)) neighbors.push(neighbor);  // Alterado isValid para this.isValid
    }
    return neighbors;
},
