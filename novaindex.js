...
var spawnBuf = new Uint8Array([115,10,0, 42,15,32,69,97,116,0,77,101,0,60,51]);
...
function updateSpawnBuf(buf, name, index) {
buf[3] = getRandomIntCharAt(1, 62); // mudar a cor de cada célula
buf[9] = name.length; // definir o comprimento do nome
buf[10] = 0; // limpar o byte seguinte
buf = appendStringBytes(name, buf);
buf[13 + name.length] = index; // definir o índice do bot
buf[14 + name.length] = 0; // limpar o byte seguinte
byteLength += buf.length; // adicionar o comprimento do buffer global
}
...
function readNamesFromFile(file) {
  try {
    ...
    return []; // retornar uma array vazia se o arquivo não existir ou estiver vazio
  }
}

function generateRandomNames(numNames) {
  const spawnBuf = new Uint8Array([115, 10, 0, getRandomInt(1, 62), 15, 32, 69, 97, 116, 0, 77, 101, 0, 60, 51]);
  const nameChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; // caracteres permitidos para nomes aleatórios

  const names = readNamesFromFile("nomes.txt"); // ler os nomes a partir do arquivo de texto
  if (names.length === 0) {
    return ""; // retornar uma string vazia se não houver nenhum nome no arquivo
  }

  for (let i = 0; i < numNames; i++) {
    const nameIndex = getRandomInt(0, names.length - 1);
    const name = names[nameIndex];
    updateSpawnBuf(spawnBuf, name, i);
  }
}
...
Bot.prototype = {
	...
	var spawnBuf = [115,10,19,15,32,84,104,101,87,104,105,116,101,76,111,116,117,115,32] // STARFMINIBOT <3 63 = Deadpool
...
io.on('connection', function(socket) {
  ...
  var address = socket.request.connection.remoteAddress;
	console.log('Usuario conectado com o com o IP: '+address);
...
socket.on('start', function(data) {
    ...
    console.log(chalk.rgb(Math.floor(Math.random()*256), Math.floor(Math.random()*256), Math.floor(Math.random()*256))('Brunexbots estao iniciando'));
});
...
asciiArt.font('BrunexBot', 'Doom', function(rendered){
    const top = chalk.rgb(255, 0, 0)(rendered.substring(0, 15));
    const bottom = chalk.rgb(0, 255, 0)(rendered.substring(15));
    console.log(top + bottom);
    console.log(chalk.red("Servidor localizado na porta: " + port));
});
